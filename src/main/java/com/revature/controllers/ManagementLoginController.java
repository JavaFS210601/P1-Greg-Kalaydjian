...
import com.revature.services.ManagementLoginService;

public class ManagementLoginController {

	ObjectMapper om = new ObjectMapper(); //so we can work with JSON
	private ManagementLoginService ManagementLoginService = new ManagementLoginService();
	...
	public void login(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if(req.getMethod().equals("POST")) { //making sure we actually got a POST request before executing. 
			...
			if(ManagementLoginService.login(lDTO.username, lDTO.password)) { //if the username/password sent to the service are valid...
